function [ligand_final] = DioP_anchor_xy(ligand,Grid_Heatmap,pocket_centroid,protein,protein_centroidA,lcp_dist_max)
...
for i = 1:size(E_grid_centroid,1)
    if (E_grid_centroid(i,1)~=0 && E_grid_centroid(i,2)~=0 && E_grid_centroid(i,3)~=0) && ( isnan(E_grid_centroid(i,1))~=1 || isnan(E_grid_centroid(i,2))~=1 || isnan(E_grid_centroid(i,3))~=1 )
...
for i = 1:size(dist_grid_cent,1)
    min_dist_grid_cent = min(dist_grid_cent(i,:));
    if min_dist_grid_cent>lcp_dist_max+1;
        dist_grid_cent(i,:) = 0;
    end
end
...
for pcnum = 1:size(pocket_centroid,1)
    ...
    for lp =1:size(ligand_pin_distance,1)
        ...
        for lg = 1:size(dist_pocket_cent,1)
            ...
            if dist_pocket_cent(lg,1)~=0 && abs(dist_pocket_cent(lg,1)-ligand_pin_distance(lp,1))<=Rmeasure
        ...
        for j=1:size(pockatomchosen_list,1)
            ...
            if ~isempty(maxdgx) && repulsenum(protein,dG_X(maxdgx(1,1)).structure)<60
