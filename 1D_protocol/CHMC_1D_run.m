...
home_dir = strcat('/directory/to/all/test/structures/');
list=dir(home_dir);
size_list=size(list);
%Aout=zeros(200,6);
%Bout=zeros(200,28);
...
run GARF_Potential
%90:92
for zz = 3:size_list(1,1)
%for zz = 9:9
    folder_dir = strcat('/directory/to/all/test/structures/',list(zz,1).name,'/');
    list1=dir(folder_dir);
    ...
    for zh=1:size_list1(1,1)
        %147:228
        tic
        if length(list1(zh,1).name)<11
            continue
        elseif length(list1(zh,1).name)>=11
            list1_name=list1(zh,1).name;
            %list_name{zh,1}=list_name;
            if strcmp(list1_name(end-11:end),'_protein.pdb')==1 %&& strcmp(list1_name(1:4),list_name)==1
                protein_id = list1_name(1:4);
                pdbid = protein_id;
                
                protein_name = list1_name;
                protein_dir = [ strcat(folder_dir,list1_name)];
                [protein,protein_list_num] = protein_define_final_wwaterclose(protein_dir);
                ligand_name = [ strcat(protein_id,'_ligand.mol2')];
                ligand_dir = [ strcat(folder_dir,ligand_name)];
                ...
                for lyr = 0:0.5:10
                    if lyr == 0
                        ...
                        pocket_protein_centroid_dist_0 = pocket_protein_centroid_dist;
                    elseif lyr > 0
                        pointgroupf = findPCB1_correct(lyr,grid_d,pocket_centroidA,protein_centroidA,pocket,protein);
                        pocket_centroid = pointgroupf(1).carpointgroupf;
                        pocket_centroid_ave(1,1) = mean(pocket_centroid(:,1));
                        pocket_centroid_ave(1,2) = mean(pocket_centroid(:,2));
                        pocket_centroid_ave(1,3) = mean(pocket_centroid(:,3));
                        
                        %[pocket_centroid,pocket_centroid_ave] = gridshell_channel(lyr,pocket_centroidA,protein_centroidA,pocket_protein_centroid_dist,protein,grid_d);
                        %[pocket_centroid,pocket_centroid_ave] = gridshell_thorough(lyr,pocket_centroidA,protein_centroidA,pocket_protein_centroid_dist,protein,grid_d);
                    ...
                    if size(pocket_centroid,1)>0
                        ...
                        [ligand_final1] = DioP_anchor_vf(ligand,Grid_Heatmap,pocket_centroid,protein,protein_centroidA);
                        ...
                        if size(dG_select_X1,1)>0
                            ...
                            Final_result_X1 = zeros(1,5);
                            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                            mkdir(strcat('/structure/output/directory/',num2str(lyr),'Angstrom_layer','/output/'));
                            ...
                            for i = 1:size(dG_select_pass_X1,1)
                                ...
                                %[ligand_final_str_X1,ligand_final_dG_X1,ligand_final_repulse_num_X1] = Erelax_NRG(ligand_select_X1,dG_select_pass_X1,i,protein,vdw_d);
                                ...
                                Final_result_X1(i,5)=dist(ligand_str_X1_centroid(1,:),pocket_centroidA(1,:));
                                0
                                
                                if Final_result_X1(i,2)<=size(ligand,1) && abs(Final_result_X1(i,5)-abs(lyr)) <=2*grid_d
                                    Final_result_X1(i,4)=1;
                                    input_struc = ligand_final_str_X1;
                                    
                                    
                                    listC = strcat(ligand_name,'_X1');
                                    output_dir = strcat('/structure/output/directory/',ligand_name,'/',num2str(lyr),'Angstrom_layer','/output/');
